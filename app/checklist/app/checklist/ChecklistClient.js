...
export default function ChecklistClient() {
  ...
  const reset = () => {
    if (confirm("Reset to default tasks?")) {
      setItems(DEFAULTS.map((t, i) => ({ id: String(i + 1), text: t, done: false })));
    }
  };
  ...
  useEffect(() => {
    ...
    if (p) {
      try {
        setItems(JSON.parse(atob(decodeURIComponent(p))));
      } catch {}
    }
  }, []);
  ...
  return (
    <>
      <div className="card">
        ...
        <p className="muted">
          Track tasks together. Your list saves in your browser. Use “Share list” to copy a link—no server needed.
        </p>
